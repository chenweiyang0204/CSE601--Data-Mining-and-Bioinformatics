...
def best_split(dataset, num_of_features):
    # unique_label = np.unique(sample[-1] for sample in dataset)
    split = None
    _score = sys.maxsize
    columns = [d for d in range(dataset.shape[1]-1)]
    col = sample(columns, num_of_features)
...
def child_split(node, num_of_features):
    left = node['left']
    right = node['right']
    node.pop('left', None)
    node.pop('right', None)
    ...
    if len(set(left[:,-1])) == 1:
        ...
        node['left'] = child_split(best_split(left,num_of_features),num_of_features)

    if len(set(right[:,-1])) == 1:
        node['right'] = terminal_node([], right)
    else:
        node['right'] = child_split(best_split(right,num_of_features),num_of_features)
    return node
...
def create_forest(train, test, num_of_tree, num_of_features):
    length_t = len(train)
    prediction = defaultdict(list)
    for idx in range(num_of_tree):
        ran_col = np.random.choice(length_t, length_t, replace=True)
        training_set = train[ran_col,:]
        starting_root = best_split(training_set,num_of_features)
        root = child_split(starting_root, num_of_features)
        # print(root)
        # sys.exit=
        for idx in range(len(test)):
            prediction[idx].append(predict(root, test[idx]))
        # print(prediction)
        # sys.exit()
    max_prediction = []
    for idx, predicts in prediction.items():
        max_prediction.append(max(set(predicts), key = predicts.count))
    # print(max_prediction)
    # sys.exit()
    return max_prediction


def classify(dataset, features, num_fold, num_of_tree, num_of_features, labels):
    ...
    for idx in range(num_fold):
        ...
        predict_list = create_forest(training_set,testing_set,num_of_tree,num_of_features)
...
if __name__ == "__main__":
    ...
    n = 10
    num_of_tree = 3
    num_of_features = int(length_features*0.2)
    result = classify(dataset, features, n, num_of_tree, num_of_features, labels)
